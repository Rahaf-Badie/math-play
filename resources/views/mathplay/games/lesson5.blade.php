...
<head>
    ...
    <title>🎮 سباق الطرح مع الاستلاف - {{ $lesson_game->lesson->name ?? 'طرح عددين ضمن 999 مع الاستلاف' }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        .answer-input:focus {
            outline: none;
            border-color: #d63031;
            box-shadow: 0 0 0 4px rgba(214, 48, 49, 0.3);
...
<body>
    <div class="container">
        <h1>🎮 سباق الطرح مع الاستلاف</h1>

        <div class="lesson-info">
            الدرس: {{ $lesson_game->lesson->name ?? 'طرح عددين ضمن 999 مع الاستلاف' }}
        </div>
        ...
        <div class="borrowing-info">
            💡 <strong>مع الاستلاف:</strong> في هذه المسائل، قد تحتاج إلى الاستلاف من المنزلة الأعلى عندما تكون المنزلة الحالية غير كافية
        </div>
        ...
        <div class="game-area">
            ...
            <div class="input-container">
                <input type="number" class="answer-input" id="answer-input" placeholder="0" min="-999" max="999">
                <button class="submit-btn" id="submitBtn" onclick="submitAnswer()">✅ تحقق</button>
            </div>

            <div class="timer-container">
                <div class="timer-display" id="timer-display">⏰ 20 ثانية</div>
                <div class="timer-bar">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const baseTime = 20; // 20 ثانية لكل سؤال (أطول لأن المسائل أصعب)
            ...
            function generateSubtractionProblem() {
                ...
                do {
                    ...
                    // التأكد من وجود استلاف في الأقل في منزلة واحدة
                    if (attempts > 100) {
                        // إذا لم نجد مسألة مع استلاف بعد 100 محاولة، ننشئ واحدة يدوياً
                        num1 = Math.floor(Math.random() * 900) + 100; // عدد من 3 منازل
                        // نضمن وجود استلاف في منزلة الآحاد
                        const ones1 = Math.floor(Math.random() * 5) + 1; // 1-5
                        const ones2 = Math.floor(Math.random() * 5) + 6; // 6-10
                        num1 = Math.floor(num1 / 10) * 10 + ones1;
                        num2 = Math.floor(num2 / 10) * 10 + ones2;
                        break;
                    }

                } while (!hasBorrowing(num1, num2) || num1 <= num2);
            ...
            // توليد خطوات الحل مع الاستلاف
            function generateSolutionSteps(a, b) {
                let steps = [];
                let result = a - b;

                steps.push(`<strong>المسألة:</strong> ${a} - ${b} = ${result}`);
                steps.push(`<strong>التحقق من المنازل:</strong>`);
                ...
                while (tempA > 0 || tempB > 0) {
                    ...
                    if (digitA < digitB) {
                        steps.push(`- في هذه المنزلة: ${digitA} أصغر من ${digitB}، نستلف من المنزلة التي تليها`);
                        digitA += 10;
                        borrow = 1;
                    }

                    const digitResult = digitA - digitB;
                    steps.push(`- ${digitA} - ${digitB} = ${digitResult}`);

                    tempA = Math.floor(tempA / 10);
                    tempB = Math.floor(tempB / 10);
                }

                steps.push(`<strong>الناتج النهائي:</strong> ${result}`);
                return steps;
            }
            ...
            function timeOut() {
                ...
                totalTimeUsed += baseTime;
            }

            // عرض خطوات الحل
            function showCalculationSteps() {
                const steps = generateSolutionSteps(num1, num2);
                let stepsHTML = `<strong>📝 خطوات الحل مع الاستلاف:</strong><br><div class="step-by-step">`;
