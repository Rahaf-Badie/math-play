...
<meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>علامات الطالب | Math&Play</title>
...
<style>
    ...
    .student-info {
      background: linear-gradient(135deg, #fff8eb 0%, #ffe7b3 100%);
      padding: 25px;
      border-radius: 16px;
      margin-bottom: 30px;
...
<body>
  ...
  <nav class="navbar navbar-expand-lg navbar-light">
    <div class="container">
      <a class="navbar-brand brand-wrap" href="{{ route('mathplay.home') }}">
      ...
      <div class="collapse navbar-collapse" id="nav">
        ...
        <div class="d-flex nav-actions gap-2">
          <a href="{{ route('mathplay.home') }}" class="btn btn-login active">الصفحة الرئيسية</a>
          <a href="{{ route('mathplay.edit_student') }}" class="btn btn-cta">تعديل البيانات</a>
...
<div class="container">
      <div class="marks-container">
        <div class="marks-header">
          <h2 class="marks-title wavy-underline">علامات الطالب</h2>
          <p class="marks-subtitle">اطلع على علاماتك في جميع الوحدات</p>
        </div>
        ...
        <p><strong>الجنس:</strong> {{ $user->gender == 'male' ? 'ذكر' : 'أنثى' }}</p>
          ...
          <h4 style="font-family: 'Lalezar', cursive; color: #0d3a66; margin-bottom: 20px;">العلامات حسب الوحدات:</h4>
            ...
            @endphp
              <li class="marks-item">
                <div class="marks-item-info">
                  {{ $semester ? $semester->name : 'بدون فصل' }} → {{ $unit->name }}
                </div>
                <div class="marks-grade {{ $mark ? '' : 'marks-grade-pending' }}">
                  {{ $mark ? $mark->score : 'لم يتم الإدخال' }}
                </div>
            </li>
        @empty
              <li class="no-marks">لا توجد وحدات لهذا الصف بعد.</li>
        @endforelse
    </ul>

    @else
          <p style="text-align: center; color: #ef4444;">خطأ: بيانات الطالب غير متوفرة. يرجى تسجيل الدخول.</p>
    @endif
